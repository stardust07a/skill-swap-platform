...
import {
  ...
  MapPin, TrendingUp, Bell, Plus, CheckCircle2, AlertTriangle
} from 'lucide-react'
...
const EmptyState = ({ icon: Icon, message, actionLabel, to }) => (
  <div className="glass rounded-2xl p-6 text-center">
    <Icon className="w-10 h-10 text-white/20 mx-auto mb-3" />
    <p className="text-white/45 text-sm">{message}</p>
    {actionLabel && to && (
      <Link to={to} className="btn-primary text-sm mt-4 inline-flex items-center gap-2">
        {actionLabel}
...
const readSettledData = (result, key) => (
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        const [matchRes, convRes, reqRes] = await Promise.allSettled([
          api.get('/matches'),
          api.get('/conversations'),
          api.get('/requests'),
        ])

        setMatches(readSettledData(matchRes, 'matches'))
        setConversations(readSettledData(convRes, 'conversations'))
        setRequests(readSettledData(reqRes, 'requests'))

        const hasFailedRequest = [matchRes, convRes, reqRes].some((result) => result.status === 'rejected')
        setDashboardWarning(hasFailedRequest ? 'Bazı dashboard bilgileri şu anda yüklenemedi.' : '')
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      {dashboardWarning && (
        <div className="glass rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4 flex items-start gap-3" role="alert">
          <AlertTriangle className="w-5 h-5 text-amber-300 shrink-0 mt-0.5" />
          <p className="text-sm text-amber-100">{dashboardWarning}</p>
        </div>
      )}
          ...
          <div className="w-full sm:w-72">
            ...
            <div className="mt-4 rounded-xl border border-white/10 bg-white/[0.03] p-3">
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs font-semibold text-white/70">Profil Durumu</p>
      ...
      >
        ...
        {teachSkills.length === 0 && learnSkills.length === 0 ? (
          <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4 text-sm text-white/45">
            Henüz beceri eklenmemiş. Profilini tamamlayarak önerilen eşleşmeleri daha doğru hale getirebilirsin.
          </div>
        ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          ...
          {matches.length === 0 ? (
            <EmptyState
              icon={Users}
              message="Eşleşme bulunamadı. Profil bilgilerini tamamlayarak önerileri güçlendirebilirsin."
        ...
        <div className="space-y-4">
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <Link to="/messages" className="text-sm text-purple-400 hover:text-purple-300" aria-label="Tüm mesajlara git">
                <ArrowRight className="w-4 h-4" />
              </Link>
            </div>
            {conversations.length === 0 ? (
              <EmptyState icon={MessageCircle} message="Henüz mesaj yok" />
            ) : (
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <Link to="/requests" className="text-sm text-purple-400 hover:text-purple-300" aria-label="Tüm taleplere git">
                <ArrowRight className="w-4 h-4" />
              </Link>
            </div>
            {requests.length === 0 ? (
              <EmptyState icon={BookOpen} message="Henüz talep yok" />
            ) : (
              <div className="space-y-2">
